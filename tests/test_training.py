...
def test_training(verbose: bool = False, num_epochs: int = 1) -> None:
    ...
    assert trainer.logged_metrics.get("train_loss_epoch") is not None, "No training loss logged"
    ...
    assert trainer.logged_metrics.get("test_loss_epoch") is not None, "No test loss logged"
    assert trainer.logged_metrics.get("test_acc_epoch") is not None, "No test accuracy logged"
